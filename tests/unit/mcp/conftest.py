"""
Shared fixtures for the V8 MCP tests.

A fresh in-memory SQLite database per test, with the
identity + MCP schemas registered. Follows the same
pattern as ``tests/unit/agents/conftest.py``.
"""
...
# Import every model the MCP contexts touch so
