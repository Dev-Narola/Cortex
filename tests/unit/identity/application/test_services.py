...
def test_authenticate_returns_access_and_refresh_tokens(db_session):
    ...
    issued = service.execute(tenant_slug="auth", email="alice@x.com", password="ValidPassword123!")
...
def test_get_current_user_resolves(db_session):
    ...
    user, tenant = service.execute(user_id=result.owner.id, tenant_id=result.tenant.id)
...
def test_revoke_api_key_sets_revoked_at(db_session):
    ...
    issued = CreateApiKeyService(db_session).execute(tenant_id=result.tenant.id, name="k")
...
def test_revoke_unknown_key_raises_not_found(db_session):
    ...
    with pytest.raises(NotFoundException):
        RevokeApiKeyService(db_session).execute(api_key_id=uuid.uuid4(), tenant_id=result.tenant.id)
...
def test_revoke_cross_tenant_key_raises_not_found(db_session):
    ...
    with pytest.raises(NotFoundException):
        RevokeApiKeyService(db_session).execute(api_key_id=issued.api_key.id, tenant_id=other.id)
