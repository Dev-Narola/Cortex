...
**Status (2026-08-29):** F10 — Ongoing Hardening — **Parts 1–2 shipped** (source-level performance baseline + bundle optimization + 3D route isolation confirmation). Parts 3–5 still pending.
...
**Next phase:** F10 Part 3 — Visual Regression Testing with Playwright (the project's Playwright config + `@playwright/test@1.49.0` are already installed; F10-P3 will set up the visual test structure + baseline screenshots + CI integration + the `Docs/frontend/f10-visual-regression.md` workflow doc).
