...
**Status (2026-08-29):** F10 — Ongoing Hardening — **Parts 1–3 shipped** (source-level performance baseline + bundle optimization + 3D route isolation confirmation + visual-regression infrastructure). Parts 4–5 still pending.
...
**Next phase:** F10 Part 4 — Analytics & Conversion Tracking (the marketing funnel is the priority: landing → hero CTA → signup → workspace creation → dashboard; live demo engagement is a separate event track; the analytics implementation should respect the privacy contract — no chat content, no document contents, no API keys, no MCP tokens, no passwords; environment separation: dev / production; the event schema goes in `Docs/frontend/analytics-events.md`).
