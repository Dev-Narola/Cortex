...
**Status (2026-08-29):** F10 — Ongoing Hardening — **All 5 parts shipped** (source-level performance baseline + bundle optimization + 3D route isolation + visual-regression infrastructure + provider-agnostic analytics abstraction + A/B testing decision + final hardening audits + F10 completion summary). **The F0–F10 frontend MVP is complete.** F10+ is intentionally open-ended per the source roadmap; F11+ candidates are listed in `Docs/frontend/f10-completion.md`.
...
* **Part 5** — A/B Testing + Final Hardening (the A/B testing decision is the spec's "no A/B infrastructure added" outcome, documented in `Docs/frontend/f10-ab-testing-decision.md`: the F10+ roadmap says A/B testing is conditional on a real named question, and Cortex currently has (1) no specific hypothesis, (2) insufficient marketing-site traffic for statistical significance, (3) no analytics provider selected yet — all three trigger conditions for a re-evaluation are documented; the "no experiment" outcome is the spec's valid completion outcome and is preferred under the "never add a technology because it's trendy" rule; the F10 final completion summary is in `Docs/frontend/f10-completion.md`, documenting every F10-Part 1–5 deliverable, the Definition-of-Done checklist (12 checked, 3 deferred with reasons), and the F11+ candidates list; the three final audits are all clean: **dependency audit** (all 22 production deps are project-required; F10-P2 already removed `framer-motion`); **environment audit** (all 8 `NEXT_PUBLIC_*` vars documented in `packages/config/src/env.ts` with Zod validation); **sensitive-data audit** (no tokens in localStorage — the auth store explicitly forbids it; no tokens in console.log — the project uses a structured logger; only WebSocket auth + reset-password tokens in URL params; no third-party analytics collecting sensitive data — the F10-P4 type system prevents it))

**Status (2026-08-29):** F10 is **complete**. The F0–F10 frontend MVP is **done**. Per the source roadmap, F10+ is intentionally open-ended — it's "ongoing hardening, not another feature phase." The 6 F11+ candidates are listed in `Docs/frontend/f10-completion.md` §"What's next (F11+)".

### F11+ — Ongoing Hardening (F10+'s next)

- Performance: Lighthouse pass, bundle-size budget, code-splitting the 3D route specifically so it never taxes the rest of the app's load time. (F10-Part 1 + 2 baseline established; Lighthouse rerun is the only remaining item.)
- Visual regression testing (Playwright screenshot diffing or Chromatic) — worth adding once the component library is stable enough that regressions are actually worth catching automatically, not before. (F10-Part 3 infrastructure shipped; baseline PNGs + CI integration are F11+ items.)
- Analytics / conversion tracking on the marketing site. (F10-Part 4 abstraction + catalog shipped; provider selection is the F11+ item.)
- A/B testing infrastructure, if and only if there's a real, named question it would answer — same "never add a technology because it's trendy" rule the backend blueprint already committed to. (F10-Part 5 decision: not yet; revisit when named hypothesis + sufficient traffic + selected provider.)
